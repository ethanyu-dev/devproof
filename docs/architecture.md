...
- Session expiry revokes permission and quarantines occupied slots/identity permits until browser closure is verified. Broad business-data locks are disabled by default. Assigned test accounts always acquire narrow environment/account/type occupancy claims; conflicting work waits until verified browser closure. Unknown-write recovery records remain independently available. `BROWSER_EXECUTION_DATA_LOCKS_ENABLED=true` additionally enables declared resource and environment-wide locks that remain held until unknown writes are reconciled. Completed outcomes release verified-closed resources.
