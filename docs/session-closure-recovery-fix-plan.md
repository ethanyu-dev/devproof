...
鉴权：现有 AuthContext 没有 role，不能只检查已登录或在前端隐藏按钮。读取沿用团队隔离；申请会话恢复和重试关闭允许当前有效团队成员操作，服务端重新查询 TeamMembership，并校验目标属于同团队。关闭证明、有效执行保护、版本校验和审计继续生效。写结果核实/节点排空仍在服务端要求 TeamMembership.role=ADMIN。若已有资源级授权更严格，则同时满足。跨团队冲突只显示受限关联标识和“共享环境存在待恢复执行”，不返回另一团队的 session/Run/目标 URL。
