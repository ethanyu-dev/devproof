...
Profile resolution does not reserve the browser. `SERIAL_PERSISTENT` retains the FIFO Task reservation and serial Cases. `ISOLATED_AUTH` reuses a verified, immutable local authentication snapshot in a separate browser/context for each Attempt; Task resolution holds no execution permit. Admission atomically acquires node capacity and a Profile concurrency permit. Shared backend data locks are optional and disabled by default. Browser closure must be verified before physical capacity is released.

## Concurrent execution

Runtime v1.13 and `BROWSER_ISOLATED_AUTH_ENABLED=true` are required. Ordinary serial login verification never starts parallel probes. To opt in, prepare or reopen the login in Console, explicitly select **Validate concurrent login** in the login window, and verify/save. The API equivalent is `POST /console/api/browser-profiles/:id/verify` with `{ "prepareIsolatedAuth": true }`; an empty body preserves serial verification. This prepares a snapshot while the Profile remains serial, so no pre-existing snapshot is needed to begin validation. Once all existing Tasks and sessions have drained, select **Independent concurrent sessions** and a limit of 1–32 in the Profile settings. Actual execution concurrency is also bounded by the assigned Runtime's available capacity.

An existing isolated Profile may raise or resave its concurrency limit while Tasks or sessions remain bound, provided its verified snapshot remains valid and no authentication settings change. Existing permits and unverified-session capacity stay reserved. Lowering the limit, switching execution modes, or changing authentication settings still requires bound Tasks to finish and sessions to be confirmed closed.
...
By default, Cases targeting the same backend can run concurrently within Runtime and Profile capacity. Explicitly assigned `executionPolicy.testAccounts` additionally coordinate by backend environment, normalized account/known alias, and required business type. Readers share a claim; a matching writer waits. An unspecified type covers all types. Claims persist hashed identities, permit account reuse, and release only after browser closure is verified. Account occupancy does not assert that an unknown write was resolved; recovery records remain available for review. Unassigned work is not serialized unless broad data locks are enabled.

Set `BROWSER_EXECUTION_DATA_LOCKS_ENABLED=true` on all API replicas to opt into business-data serialization. In that mode, `READ_ONLY` work may share locks; `MUTATING` work locks its declared resource scopes; unknown work locks the backend environment exclusively. Root/collection scopes conflict with their descendants. Set `BROWSER_EXECUTION_ENVIRONMENTS_JSON` for hostname aliases backed by the same server state. Distinct users, Profiles, or Task deployment IDs do not partition those optional locks. Case execution declarations come from explicit request policy or Console review, not generated test prose.

Issue requests can provide `caseExecutionPolicies` keyed by generated Case ID or one-based position. Set `casePolicyReviewRequired=true` to pause after generation and review each Case's access mode, resource scopes, and dependencies before creating its immutable Run. Unknown access mode does not serialize automatic Tasks unless business-data locks are enabled. Explicit dependencies, Profile authorization and browser capacity apply in either mode.
