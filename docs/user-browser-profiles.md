...
By default, Cases targeting the same backend run concurrently whenever Runtime and Profile capacity is available, including `MUTATING` and `UNKNOWN` work. An unresolved write outcome remains attached to its own execution and does not block other executions. Existing quarantined business-data leases are retained for reconciliation but ignored by admission. Concurrent tests must coordinate shared business records through their test data or explicit Case dependencies.
