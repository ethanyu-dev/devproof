...
执行详情和任务详情中的“重试用例”复用原 Spec，在原任务下新增一次 Case 执行，`executionOrdinal` 递增。历史执行与证据保留，任务列表不增加 Issue 任务；统计和最终结果按各用例、各环境的最新执行汇总。没有待处理恢复记录时直接提交；有写入结果未知的记录时，旧浏览器必须先确认关闭，再由原任务发起人或团队管理员点击一次“确认重试”，不要求填写业务核实结果、说明或证据。

`POST /console/api/runtime-recoveries/:id/authorize-retry` 接收 `expectedVersion`、`idempotencyKey` 和显式的 `acknowledgeUnknownWrite: true`。服务在资源锁下核验团队成员身份、原任务发起人或管理员权限、会话关闭证明、会话版本及原执行已结束，记录 `runtime.retry.authorized` 审计，释放该恢复记录的数据保护。状态记为 `RETRY_AUTHORIZED`，保留原执行失败结果，`writeResolvedAt` 仍为空；不会把用户的重试授权标记成“已证实没有写入”或“已核实业务结果”。

业务核实仍可在恢复详情独立提交，也可在授权重试后补充。授权和用例重跑各自保留幂等标识，响应丢失后可继续提交。浏览器未关闭、执行仍活动、版本冲突或权限不足时不会放开重试。现有状态字段是字符串，无需数据库迁移；部署时先构建 Contracts，再更新 API 和 Web。

`POST /console/api/tasks/:id/cases/:caseId/rerun` 接收可选 `idempotencyKey`，在资源锁内通过任务事件去重，返回原任务详情。旧 `/rerun-task` 入口兼容为相同的原任务内重跑行为；Tool API 也保持一致。每次有效重跑按原配置从当前时间重新提供执行时限，且不缩短任务已有的截止时间。Spec 分析不重新执行，其他用例不新增执行。整任务“重跑任务”仍表示创建新的 Issue 任务。
