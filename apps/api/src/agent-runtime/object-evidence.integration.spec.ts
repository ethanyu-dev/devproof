...
it("verifies three real modal defaults, persists their screenshots, compares and submits bound evidence", async () => {
  ...
  try {
    ...
    for (const target of [contract.targets[0]!, contract.targets[2]!]) {
      ...
      freshIds.push(
        selected.bound.bindings.find((b) => b.targetId === target.targetId)!.id,
      );
      await execute("page.click", { target: { selector: "#close" } }, true);
    }
