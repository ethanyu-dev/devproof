...
- A unified task flow: Issue / PR / testing brief → Task → AI Spec Analysis → Profile Resolution → Spec Execution; direct tasks skip the first two stages
...
6. Spec analysis for Spec Tasks is executed by Agent Runtime. The Agent reads selected Issue/PR/brief context and PR metadata, diffs, and code pinned to the PR head SHA through credential-isolated read-only control-plane tools, then submits an immutable source-cited Spec. Protocol v2.17 uses a compact Case draft and persists `agent-spec-v3` with explicit requirement coverage; older workers retain `agent-spec-v2`. See [the protocol notes](packages/agent-runtime-protocol/README.md). Configure `SPEC_ANALYSIS_MODE=AGENT`; use `SHADOW` to record a comparison with the legacy deterministic generator or `DETERMINISTIC` for rollback. Linear prefers the official GraphQL API through `LINEAR_API_TOKEN`, with `LINEAR_MCP_BEARER_TOKEN` as a fallback. Configure encrypted GitHub PAT entries in Console → Access. Every model turn, structured analysis summary, tool call, validation correction, and final Spec is recorded in the Task trajectory; raw hidden chain-of-thought and credentials are never recorded.
