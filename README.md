...
Agent models must support the OpenAI-compatible Chat Completions API: Runtime appends `/chat/completions` to the configured Base URL (usually ending in `/v1`). Both pools use `tool_choice: auto` and require tool-based completion; reasoning fields are preserved in memory between tool turns.

6. Spec analysis for Issue Tasks is executed by Agent Runtime. The Agent reads the Issue, linked PR metadata, diffs, and code pinned to the PR head SHA through credential-isolated read-only control-plane tools, then submits an immutable source-cited Spec. Protocol v2.17 uses a compact Case draft and persists `agent-spec-v3` with explicit requirement coverage; older workers retain `agent-spec-v2`. See [the protocol notes](packages/agent-runtime-protocol/README.md). Configure `SPEC_ANALYSIS_MODE=AGENT`; use `SHADOW` to record a comparison with the legacy deterministic generator or `DETERMINISTIC` for rollback. Linear prefers the official GraphQL API through `LINEAR_API_TOKEN`, with `LINEAR_MCP_BEARER_TOKEN` as a fallback. Configure encrypted GitHub PAT entries in Console → Access. Every model turn, structured analysis summary, tool call, validation correction, and final Spec is recorded in the Task trajectory; raw hidden chain-of-thought and credentials are never recorded.
