...
API, Web, and Agent Runtime use their respective Dockerfiles. Before each API deployment starts, Railway runs `pnpm prisma:deploy`; a failed migration stops the release. Create two Railway services from the Agent Runtime config and give each its pool-specific token. The Browser Execution service automatically reconciles its worker lanes to the sum of online Browser node capacities (for example, node A `4` plus node B `8` gives a schedulable pool of `12`). Domain-routed work waits only in its target node queue; unmatched work uses the flexible queue and prefers idle compatible nodes.
