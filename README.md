...
5. In Console → Access → Agent Models, configure the team-wide ordered model list. Each entry contains a Base URL, encrypted API Key, Model ID, and Display Name. The list order controls fallback and recovery priority. The Agent Runtime Token is a separate deployment identity: run `pnpm --filter @devproof/api runtime:provision -- --team default`, then store the one-time token as `DEVPROOF_AGENT_RUNTIME_TOKEN` in the Runtime process. Operators can approve an exact private or HTTP model gateway with `DEVPROOF_AGENT_MODEL_HOST_ALLOWLIST`. Restart `pnpm dev`; Web and API always start, while the Agent Runtime starts when its internal credential is present.

6. Spec analysis for Issue Tasks prefers the official Linear GraphQL API through `LINEAR_API_TOKEN`, with `LINEAR_MCP_BEARER_TOKEN` as a fallback. For issue-owner Profile mapping, also configure `LINEAR_WORKSPACE_ID`; use the stable Linear user ID as the primary identity and a unique verified email only as a one-time backfill fallback. Configure encrypted GitHub PAT entries in Console → Access, scoped by organization or exact repository and ordered by priority, to enrich the snapshot with PRs, checks, files, and deployments. Knowledge MCP is optional; when using RAGFlow, set `KNOWLEDGE_MCP_TOOL` to a read-only retrieval tool.
...
When a Task reaches a terminal state, the control plane replies to the original Feishu message through a durable outbox, or through a group bot webhook, and idempotently writes the same summary to the linked GitHub PR. Redelivery updates the existing marked task comment instead of creating duplicates. The notification links to the final result, including per-step screenshots and the action video stored in R2. The matching Console-managed GitHub PAT needs permission to write Issue/PR comments in the target repository.
