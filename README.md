...
Build the daemon and install the pinned Chromium version on the Runtime host:

    pnpm --filter @devproof/browser-runtime build
    pnpm --filter @devproof/browser-runtime install:browser

In Console → Access → Execution Runtime, select Register, copy the generated one-time command, and run it on the target host. After pairing succeeds, start the daemon:
...
Run the following from a development machine. The script builds Runtime once, verifies the package, and distributes it over SSH. Existing hosts preserve `runtime.json` and their systemd configuration, warm the new package and browser while the old service is still online, verify that no Browser Session is active before switching, then upgrade in place and confirm that the Runtime reconnects:

    pnpm deploy:browser-runtime -- \
      --identity ~/.ssh/devproof-browser-runtime \
      ppuser@runtime-host-1 ppuser@runtime-host-2

For a new Linux host, provide the one-time pairing token generated by Console and the API address during the first installation. The token is passed through SSH standard input and is never written to the command line, package, or remote disk:
...
A new Ubuntu/Debian host must allow a regular user to log in with an SSH key, use the systemd user manager, and access the Node.js download source, npm registry, Playwright CDN, DevProof API, and Runtime Gateway. The first connection records a new SSH host key; a changed key for a known host is rejected. The script installs a user-local Node.js 24 distribution, Chromium, system dependencies, and the Runtime service when needed. Initial installation requires passwordless sudo for that user. An upgrade is rejected when active sessions are detected unless interruption risk is explicitly accepted with `--force-active`.

Configure `DEVPROOF_RUNTIME_NAME`, `DEVPROOF_MAX_CONCURRENCY`, and `DEVPROOF_HEADLESS` in `~/.config/devproof/browser-runtime.env`, then rerun the deployment command. `DEVPROOF_MAX_CONCURRENCY` is used only as the host's initial capacity during first registration. Every successful installation records the package hash and timestamp in `~/.devproof-browser-runtime/install.json`. After the node is bound, use Console → Access → Runtime Access and Capacity to set the authoritative concurrency and allowed private-network hosts for each Runtime. The default block policy remains active when the allowlist is empty.

Private-network hosts may use aliases from `~/.ssh/config` or repeated `--ssh-option` arguments such as `--ssh-option ProxyJump=user@bastion` and `--ssh-option Port=2222`. The options are passed to both `ssh` and `scp`.
