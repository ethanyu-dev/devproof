...
Options:
  ...
  --direct-config FILE  Validate and persist direct-access JSON configuration.
                        Omit to preserve all current direct-access settings.
    ...
    if [[ "$upgrade_started" == true ]]; then
      ...
      if [[ "$environment_changed" == true ]]; then
        if [[ "$environment_existed" == true ]]; then
          cp -p -- "$environment_backup" "$ENV_PATH"
        else
          rm -f -- "$ENV_PATH"
        fi
      fi
...
direct_config_helper="$staging_dir/node_modules/@devproof/browser-runtime/dist/install-direct-access.js"
if [[ -n "$DIRECT_CONFIG" ]]; then
  [[ -f "$direct_config_helper" ]] ||
    die "this Runtime release does not support --direct-config."
  # Freeze the input before stopping the current service; never source JSON or env.
  cp -- "$DIRECT_CONFIG" "$staging_dir/direct-config.json"
  DIRECT_CONFIG="$staging_dir/direct-config.json"
  node "$direct_config_helper" stage "$DIRECT_CONFIG" "$ENV_PATH" \
    "$staging_dir/browser-runtime.env"
fi
...
if [[ -n "$DIRECT_CONFIG" ]]; then
  # Re-read the live file so unrelated operator changes during staging survive.
  rm -- "$staging_dir/browser-runtime.env"
  node "$direct_config_helper" stage "$DIRECT_CONFIG" "$ENV_PATH" \
    "$staging_dir/browser-runtime.env"
  mkdir -p "$(dirname "$ENV_PATH")"
  if [[ -f "$ENV_PATH" ]]; then
    environment_existed=true
    environment_backup="$(mktemp "$ENV_PATH.before-direct.XXXXXX")"
    cp -p -- "$ENV_PATH" "$environment_backup"
    chmod 0600 "$environment_backup"
  fi
  environment_temporary="$(mktemp "$ENV_PATH.pending.XXXXXX")"
  cp -- "$staging_dir/browser-runtime.env" "$environment_temporary"
  chmod 0600 "$environment_temporary"
  environment_changed=true
  mv -f -- "$environment_temporary" "$ENV_PATH"
  environment_temporary=""
fi
...
if [[ ! -f "$STATE_PATH" ]]; then
  ...
  if [[ -n "$DIRECT_CONFIG" ]]; then
    printf 'Direct settings are saved; pair before checking the listener and configuring the API endpoint.\n'
  fi
  cleanup_files
  trap - EXIT HUP INT TERM
  exit 0
...
if [[ -n "$DIRECT_CONFIG" ]]; then
  node "$direct_config_helper" verify "$DIRECT_CONFIG"
  node "$direct_config_helper" describe "$DIRECT_CONFIG" "$STATE_PATH"
fi
