...
This script is normally invoked by scripts/deploy-browser-runtime.sh.
...
if [[ "$new_device" == true && -z "$PAIR_API" ]]; then
  die "this device is not paired; rerun the deploy command with --pair-api and --pair-token."
fi
if [[ "$new_device" == false && -n "$PAIR_API" ]]; then
  die "this device is already paired; omit --pair-api and --pair-token."
fi
...
fs.writeFileSync(
  path,
  JSON.stringify(
    { installedAt: new Date().toISOString(), packageSha256, runtimeId, version },
    null,
    2,
  ) + "\n",
  { mode: 0o600 },
