...
先确定 Issue 明确要求和实际改动直接影响的行为，再固定 requirements。Route Spec、整页文档、未改动代码是理解行为的依据，不会自动成为本次验收范围。需求依据来自次级来源时，必须提供 changeBasis：引用已读 Issue 或 diff 的原文，并用 reason 说明该行为与本次需求/改动的具体关系；diff 引用须包含实际增删行（保留 +/-）。不能只写“文档有定义”或“属于同一页面”。
...
默认不追加通用权限、分页、排序、完整 CRUD、重复启停、逐字段 POST/PUT 校验或独立的重复只读 Case。只有 Issue 明确要求或实际变更直接影响时才纳入。登录、打开页面、定位控件、获取测试账号、截图和清理属于执行准备/操作/取证，不单列产品验收；清理和数据归属要求仍须执行。
...
export function requirementNecessityError(
  ...
  if (source.kind === "LINEAR_ISSUE" && !proof) return null;
  if (!proof)
    return `需求 ${requirement.id} 来自次级来源，缺少 changeBasis。请引用 Issue 或实际 diff 并说明本次必须验证的原因；仅属页面已有能力的内容放入 outOfScope，不生成必需验收。`;
  const kind = sources.get(proof.sourceRef)?.kind;
  if (kind !== "LINEAR_ISSUE" && kind !== "GITHUB_DIFF")
    return `需求 ${requirement.id} 的 changeBasis 必须引用已读取的 Issue 或实际 diff，整页文档、实现文件和 PR 描述不能独自扩展范围。`;
